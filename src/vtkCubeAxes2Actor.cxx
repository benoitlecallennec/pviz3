...
vtkStandardNewMacro(vtkCubeAxes2Actor);
vtkCxxSetObjectMacro(vtkCubeAxes2Actor, Camera,vtkCamera);
...
vtkCubeAxes2Actor::vtkCubeAxes2Actor() :
...
void vtkCubeAxes2Actor::ShallowCopy(vtkCubeAxes2Actor *actor)
{
...
int vtkCubeAxes2Actor::RenderOpaqueGeometry(vtkViewport *viewport)
{
...
void vtkCubeAxes2Actor::AdjustAxes(double bounds[6], double xCoords[4][6],
...
void vtkCubeAxes2Actor::ReleaseGraphicsResources(vtkWindow *win)
{
    for (int i = 0; i < 4; i++)
    {
...
// *************************************************************************
void vtkCubeAxes2Actor::GetBounds(double bounds[6])
{
    for (int i=0; i< 6; i++)
    {
...
// Compute the bounds
void vtkCubeAxes2Actor::GetBounds(double& xmin, double& xmax,
...
// Compute the bounds
double *vtkCubeAxes2Actor::GetBounds()
{
    return this->Bounds;
}
...
void vtkCubeAxes2Actor::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkCubeAxes2Actor::TransformBounds(vtkViewport *viewport,
...
bool vtkCubeAxes2Actor::ComputeTickSize(double bounds[6])
{
...
// ****************************************************************************
//  Method: vtkCubeAxes2Actor::AdjustValues
//
//  Purpose:
...
void vtkCubeAxes2Actor::AdjustValues(const double xRange[2],
...
// ****************************************************************************
//  Method: vtkCubeAxes2Actor::AdjustRange
//
//  Purpose:
...
void vtkCubeAxes2Actor::AdjustRange(const double bnds[6])
{
...
int vtkCubeAxes2Actor::Digits(double min, double max )
{
...
int vtkCubeAxes2Actor::LabelExponent(double min, double max)
{
    if (min == max)
    {
        return 0;
...
void vtkCubeAxes2Actor::BuildAxes(vtkViewport *viewport)
{
    double bounds[6];
    double pts[8][3];
    int i;
        ...
        this->ForceZLabelReset)
    {
        ...
        double labelscalexfactor = labelscale * labelScaleFactor;
...
void vtkCubeAxes2Actor::SetNonDependentAttributes()
{
...
//
static int vtkCubeAxes2ActorTriads[8][3] = {
    {mm,mm,mm}, {mm,Xm,Xm}, {Xm,mm,mX}, {Xm,Xm,XX},
    {mX,mX,mm}, {mX,XX,Xm}, {XX,mX,mX}, {XX,XX,XX}};
static int vtkCubeAxes2ActorConn[8][3] = {{1,2,4}, {0,3,5}, {3,0,6}, {2,1,7},
...
// *************************************************************************
void vtkCubeAxes2Actor::DetermineRenderAxes(vtkViewport *viewport)
{
    ...
    if (this->RenderCount++ == 0 || !(this->RenderCount % this->Inertia))
    {
        if (this->FlyMode == VTK_FLY_CLOSEST_TRIAD)
        {
            ...
            xloc = vtkCubeAxes2ActorTriads[idx][0];
            yloc = vtkCubeAxes2ActorTriads[idx][1];
            zloc = vtkCubeAxes2ActorTriads[idx][2];
        ...
        else
        {
            ...
            {
                num = (pts[vtkCubeAxes2ActorConn[idx][i]][1] - pts[idx][1]);
                den = (pts[vtkCubeAxes2ActorConn[idx][i]][0] - pts[idx][0]);
                if (den != 0.0)
                {
                    slope = num / den;
                }
                ...
                {
                    xIdx = vtkCubeAxes2ActorConn[idx][i];
                    yIdx = vtkCubeAxes2ActorConn[idx][(i+1)%3];
                    zIdx = vtkCubeAxes2ActorConn[idx][(i+2)%3];
                    xAxes = i;
                    minSlope = slope;
                }
            }
            ...
            zAxes = (xAxes != 0 && yAxes != 0 ? 0 :
                     (xAxes != 1 && yAxes != 1 ? 1 : 2));
            if (pts[vtkCubeAxes2ActorConn[xIdx][zAxes]][2] <
                pts[vtkCubeAxes2ActorConn[yIdx][zAxes]][2])
            {
                zIdx = xIdx;
                zIdx2 = vtkCubeAxes2ActorConn[xIdx][zAxes];
            }
            else
            {
                zIdx = yIdx;
                zIdx2 = vtkCubeAxes2ActorConn[yIdx][zAxes];
            }
            ...
            {
                case 0:
                    xloc = vtkCubeAxes2ActorTriads[mini][0];
                    break;
                case 1:
                    yloc = vtkCubeAxes2ActorTriads[mini][1];
                    break;
                case 2:
                    zloc = vtkCubeAxes2ActorTriads[mini][2];
                    break;
            }
...
double vtkCubeAxes2Actor::MaxOf(double a, double b)
{
    return (a > b ? a : b);
}

double vtkCubeAxes2Actor::MaxOf(double a, double b, double c, double d)
{
    return this->MaxOf(this->MaxOf(a, b), this->MaxOf(c, d));
}

inline double vtkCubeAxes2Actor::FFix(double value)
{
    int ivalue = static_cast<int>(value);
    return ivalue;
}

inline double vtkCubeAxes2Actor::FSign(double value, double sign)
{
    value = fabs(value);
    if (sign < 0.)
    {
...
// *******************************************************************
// Method: vtkCubeAxes2Actor::AdjustTicksComputeRange
//
...
void vtkCubeAxes2Actor::AdjustTicksComputeRange(vtkAxisActor *axes[4],
                                               double boundsMin, double boundsMax)
{
    ...
    //qDebug() << "major, minor : " << major << "," << minor;
    //qDebug() << "majorStart, minorStart : " << majorStart << "," << minorStart;
...
void vtkCubeAxes2Actor::BuildLabels(vtkAxisActor *axes[4])
{
    char label[64];
    int i, labelCount = 0;
...
//
// ****************************************************************************
void vtkCubeAxes2Actor::SetLabelScaling(bool autoscale, int upowX, int upowY,
                                       int upowZ)
{
...
/*
void vtkCubeAxes2Actor::SetTicksDelta(vtkAxisActor *axes[4],
                                     double boundsMin, double boundsMax,
